...
import { createSession, createChallengeInstance, getSession, getChallengeInstance, recommendForScene, getUserProgress, getArtworksForUser, completeSession, getMockScene, updateSessionScene, addArtwork } from './services/sessionService.js';
...
app.post('/api/sessions/:sessionId/complete', async (req, res) => {
    try {
        ...
        const result = await completeSession(req.params.sessionId, req.body.userId ?? session.userId, req.body.challengeType, req.body.artworkMetadata ?? req.body.metadata);
