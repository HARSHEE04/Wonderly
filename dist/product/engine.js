export const defaultChallengeTemplates = [
    {
        id: 'tpl-character',
        type: 'character',
...
const ingredientTypeToProperty = {
    color: 'colors',
    shape: 'shapes',
    texture: 'textures',
    line: 'lines',
...
export function validateSceneAnalysis(scene) {
...
export function getEligibleChallenges(scene, templates) {
    validateSceneAnalysis(scene);
    const eligible = [];
    const rejected = [];
    for (const template of templates.filter((item) => item.enabled)) {
        const required = template.requiredIngredientTypes;
...
export function getUnderusedConcepts(conceptExposure) {
...
export function summarizeExposure(conceptExposure) {
    return { ...defaultConceptExposure, ...conceptExposure };
}
export function buildPersonalizationContext(history) {
...
export function scoreChallenge(template, scene, history = { challengeTypes: {}, conceptExposure: {} }) {
    validateSceneAnalysis(scene);
...
export function recommendChallenge(scene, templates = defaultChallengeTemplates, history = { challengeTypes: {}, conceptExposure: {} }) {
    validateSceneAnalysis(scene);
