...
async function seed() {
  ...
  if (!connected || !isMongoConnected()) {
    console.warn(
      'MongoDB is not reachable. Configure MONGODB_URI and ensure MongoDB is running, then re-run "npm run seed".'
    );
    process.exit(0);
  }

  for (const template of seedChallengeTemplates) {
    await ChallengeTemplateModel.findOneAndUpdate(
  ...
  await UserModel.findOneAndUpdate(
    { externalId: demoUser.id },
    { externalId: demoUser.id, displayName: demoUser.displayName, preferences: demoUser.preferences },
    { upsert: true, new: true }
  );
  ...
  for (const resource of demoResources) {
    await LearningResourceModel.findOneAndUpdate(
      { concept: resource.concept.toLowerCase(), url: resource.url },
      { ...resource, concept: resource.concept.toLowerCase(), retrievedAt: new Date() },
      { upsert: true, new: true }
    );
  }
