...
describe('backend API', () => {
  ...
  it('records learning progress through the session lifecycle and exposes it via the progress endpoint', async () => {
    ...
    let colorProgress = progressResponse.body.data.learning.concepts.find((c: any) => c.concept === 'color');
    ...
    colorProgress = progressResponse.body.data.learning.concepts.find((c: any) => c.concept === 'color');
    expect(colorProgress?.timesSeen).toBe(1);
