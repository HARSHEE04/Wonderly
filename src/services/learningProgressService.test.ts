...
describe('learningProgressService', () => {
  ...
  it('feeds concept exposure into challenge scoring so underused concepts get a boost', async () => {
    ...
    const scoreCompositionWithExposure = scoreChallenge(
      compositionTemplate,
      mockScenes.architectureFriendly,
      historyWithExposure
    );
    const scoreCompositionWithoutExposure = scoreChallenge(
      compositionTemplate,
      mockScenes.architectureFriendly,
      historyWithoutExposure
    );

    // A heavily-practiced concept should receive a smaller concept boost than
    // an unpracticed one, all else equal.
    expect(scoreCompositionWithExposure.breakdown.conceptBoost).toBeLessThan(
      scoreCompositionWithoutExposure.breakdown.conceptBoost
    );
